    ...
    *,
    ...
    for attempt in range(total_attempts):
        raw = generator.generate_json(
            build_prompt(question, context, repair_note=repair_note),
            ANSWER_JSON_SCHEMA,
        )
        try:
        ...
        return RepairResult(draft=draft, events=events, attempts=attempt + 1)
    return RepairResult(
        draft=None, events=events, attempts=total_attempts, format_failed=True
    )
...
def _abstained_answer(
    *, model_id: str, variant: str, routing: RoutingDecision, events: list[GuardrailEvent]
) -> Answer:
    return Answer(
